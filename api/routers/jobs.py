...
async def download_tailored_resume(job_hash: str) -> FileResponse:
    ...
    Purpose:
        ...
        PDFs generated by the tailor worker. The app is a single-user local
        deployment — the user already chose port exposure when they ran
        `docker compose up`, so the endpoint does not impose its own access
        gate. Operators who want real auth put a reverse proxy in front.
    Args:
