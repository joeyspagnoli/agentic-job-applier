 ...
 *
 * ISO 3166 country options for candidate-profile citizenship selection.
 */

/** One selectable country option. */
...
/** Countries shown at the top of citizenship dropdowns. */
export const PRIORITY_COUNTRY_CODES = ["US"] as const;

/** Full country list sorted alphabetically (U.S. normalized label). */
...
/**
 * Build country options with priority countries pinned first.
 *
...
export function buildPrioritizedCountryOptions(): readonly CountryOption[] {
  ...
  const remainingOptions = ALL_COUNTRY_OPTIONS.filter(
    (option) => !priorityCodeSet.has(option.code),
  );
