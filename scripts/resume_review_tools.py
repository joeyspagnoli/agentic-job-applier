#!/usr/bin/env python3
"""Expose resume-review helper functions as deterministic CLI tools.

Purpose:
    Provide a tools-first command surface for pi-coding-agent review runs,
    including baseline tailor tools plus review-specific analysis/report tools.
"""
...
from src.agents.resume_review_pi.tools import analyze_latex_log_tool
from src.agents.resume_review_pi.tools import analyze_pdf_geometry_tool
from src.agents.resume_review_pi.tools import compare_pdf_to_base_tool
from src.agents.resume_review_pi.tools import extract_pdf_text_signals_tool
from src.agents.resume_review_pi.tools import write_review_report_tool
...
def _parse_report_payload(
    *,
    ...
    """Parse the report JSON payload for `write-review-report`.

    Purpose:
        Keep report-write command inputs deterministic and validated before file
        writes occur.
    Args:
        report_json: Raw report JSON string argument.
    Output:
        Returns parsed report payload mapping.
    Raises:
        ValueError: When JSON is invalid or not an object.
    """

    try:
        parsed_payload = json.loads(report_json)
    except json.JSONDecodeError as exc:
        raise ValueError(f"Invalid --report-json payload: {exc}") from exc

    if not isinstance(parsed_payload, dict):
        raise ValueError("Review report payload must be a JSON object")
    return parsed_payload


def build_parser() -> argparse.ArgumentParser:
    """Build the argparse parser for resume-review tool commands.
    ...
    parser = argparse.ArgumentParser(description="Resume review tool commands")
    ...
    geometry_parser = subparsers.add_parser(
        ...
        help="Analyze first-page PDF geometry and density signals",
    )
    geometry_parser.add_argument("--pdf-path", type=str, required=True)
    geometry_parser.add_argument("--dpi", type=int, default=150)

    compare_parser = subparsers.add_parser(
        "compare-pdf-to-base",
        help="Compare candidate PDF metrics against base PDF metrics",
    )
    compare_parser.add_argument("--candidate-pdf", type=str, required=True)
    compare_parser.add_argument("--base-pdf", type=str, required=True)
    compare_parser.add_argument("--base-geometry-json", type=str)

    log_parser = subparsers.add_parser(
        "analyze-latex-log",
        help="Analyze LaTeX log warning/error signals",
    )
    log_parser.add_argument("--log-path", type=str, required=True)

    signals_parser = subparsers.add_parser(
        "extract-pdf-text-signals",
        help="Extract text-level PDF quality signals",
    )
    signals_parser.add_argument("--pdf-path", type=str, required=True)

    report_parser = subparsers.add_parser(
        "write-review-report",
        help="Validate and write canonical review report JSON",
    )
    report_parser.add_argument("--path", type=str, required=True)
    report_parser.add_argument("--report-json", type=str, required=True)

    return parser


def main() -> int:
    """Execute one resume-review tool subcommand.
    ...
    try:
        ...
        if args.command == "analyze-pdf-geometry":
            metrics = analyze_pdf_geometry_tool(
                pdf_path=args.pdf_path,
                dpi=args.dpi,
            )
            return _emit_success(metrics.model_dump(mode="json"))

        if args.command == "compare-pdf-to-base":
            comparison = compare_pdf_to_base_tool(
                candidate_pdf=args.candidate_pdf,
                base_pdf=args.base_pdf,
                base_geometry_json=args.base_geometry_json,
            )
            return _emit_success(comparison.model_dump(mode="json"))

        if args.command == "analyze-latex-log":
            log_metrics = analyze_latex_log_tool(log_path=args.log_path)
            return _emit_success(log_metrics.model_dump(mode="json"))

        if args.command == "extract-pdf-text-signals":
            text_signals = extract_pdf_text_signals_tool(pdf_path=args.pdf_path)
            return _emit_success(text_signals.model_dump(mode="json"))

        if args.command == "write-review-report":
            report_payload = _parse_report_payload(report_json=args.report_json)
            report_path = write_review_report_tool(
                path=args.path,
                report_payload=report_payload,
            )
            return _emit_success({"path": report_path})
