...
- Purpose: Discover job postings from multiple sources (Greenhouse, Workday via Apify, JobSpy), deduplicate, store in SQLite, run an ADK agent to decide whether to apply, and autonomously tailor resumes for qualified jobs via pi-mono coding agent.
...
- `scripts/`: operational CLIs (query/find/test/decide/process), one-shot pipeline command `run_pipeline_once.py`, autonomous tailor worker `process_qualified_jobs.py`, and resume-tailor scripts (`migrate_resume_tex_to_yaml.py`, `resume_tailor_tools.py`, `run_resume_tailor.py`)
...
## Data & Persistence
- SQLite schema for `job_postings`, `crawl_history`, `daily_stats`, `tailor_runs`; queue retry fields and indexes are migrated automatically by `DatabaseManager`.
...
## Deployment
- systemd producer timer/service (`job-discovery.*`) + continuous gate worker (`job-agent-worker.service`) + continuous tailor worker (`job-tailor-worker.service`) for autonomous runtime.
- Tailor worker requires: pi-mono (or `PI_CODING_AGENT_COMMAND`), texlive-full, latexmk.
