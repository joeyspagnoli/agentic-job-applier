...
## Data & Persistence
- **DatabaseManager**: Async SQLite wrapper with configurable journal mode, queue reads, decision persistence, retry state transitions, terminal failure state, and requeue helper.
- **Schema**: job queue + crawl/stats tables plus retry fields (`agent_retry_count`, `agent_next_retry_at`).
...
- **Deployment**: timer+service producer (`job-discovery.*`) plus continuous consumer (`job-agent-worker.service`) and optional `job-agent-alert@.service`.
