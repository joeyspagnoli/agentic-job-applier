"""Apply-stage worker loop for the autonomous job-application pipeline.

Runs inside ``LoopSupervisor`` as an asyncio task when ``automation.apply_mode``
is ``autonomous`` or ``both``. Idles without claiming whenever host Chrome is
unreachable over CDP. Respects ``SAFE_MODE=true`` as a global kill switch that
prevents form submission regardless of the binary submit gate outcome.
"""
...
from src.agents.apply_worker.browser import apply_to_job as apply_to_job
from src.agents.apply_worker.browser import (
    check_chrome_reachable as check_chrome_reachable,
)
